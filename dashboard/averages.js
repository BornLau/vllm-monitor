(() => {
  ...
  async function load() {
    ...
    try {
      ...
      el('average-rows').innerHTML = data.rows.length ? `<div class="average-table-wrap"><table class="average-table"><thead><tr><th>模型</th><th>输入平均 tok/s</th><th>输出平均 tok/s</th><th>有效时长（输入 / 输出）</th></tr></thead><tbody>${data.rows.map(row=>`<tr><td title="${escape(row.project+' · '+row.node+' · '+row.model)}">${escape(row.display_model||row.model)}</td><td>${number(row.input_average)}</td><td>${number(row.output_average)}</td><td>${duration(row.input_active_seconds)} / ${duration(row.output_active_seconds)}<small>数据覆盖 ${coverage(row.input_observed_seconds)} / ${coverage(row.output_observed_seconds)}</small></td></tr>`).join('')}</tbody></table></div>` : '<div class="chart-empty">当前没有可统计的服务入口</div>';
